...
* **开发者自我测试义务 (NO USER TESTING)**：严禁在未经过自身验证的情况下让用户进行测试。在交付任何修改前，必须先利用 `run_command` (运行测试脚本) 或 `browser_subagent` (模拟 UI 操作) 进行全链路验证，确保功能在该环境内 100% 可用且逻辑正确，严禁将未经验证的代码交付给用户。
* **强制文档先行原则**：所有修改、更新或新功能的创建，**必须**同步更新相关技术文档（如 `technical_design.md` 或 `technical_design_ZH.md`）。在执行任何代码修改前，必须先根据操作进行原子化任务拆解，并在技术文档或方案中明确说明操作意图与影响。
