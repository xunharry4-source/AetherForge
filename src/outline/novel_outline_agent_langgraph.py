...
class OutlineState(TypedDict):
    ...
    autonomy_level: str    # "safe", "balanced", "aggressive"

def outline_retriever(state: OutlineState, config: RunnableConfig):
    """
...
def outline_planner(state: OutlineState, config: RunnableConfig):
    """大纲策划节点 (支持全局与分章细化)"""
    ...
    if is_chapter_detail:
        from src.common.lore_utils import get_latest_book_outline
    ...
    mode_desc = "【当前任务：全局书籍策划】请生成完整的书籍级结构化大纲。"
    schema = """{
  "meta_info": {"title": "...", "genre": "...", "tone": "..."},
  "core_hook": "...",
  "character_roster": [],
  "plot_beats": {},
  "chapter_list": []
    ...
    if is_chapter_detail:
        mode_desc = "【当前任务：单章细化】请生成单章级结构化大纲。"
        schema = """{
  "chapter_info": {"chapter_num": 1, "title": "...", "summary": "..."},
  "plot_beats": [],
  "worldview_alignment": "..."
}"""
    
    if user_feedback and prev_proposal:
        if refinement_mode == "patch":
            # 进入“精准修订模式”
            mode_desc += f"""
【！！！当前任务：精准修订 (Patch Mode)！！！】
要求：
1. 严禁洗牌式重写。
2. **锚定效应**：严禁修改与用户需求无关的章节标题、既定梗概。
3. 你的修改必须像“局部手术”一样精准。
...
要求：你可以根据反馈重新架构大纲，但需保持与世界观设定的一致性。
...
def outline_defense_node(state: OutlineState, config: RunnableConfig):
    ...
    try:
        ...
        error_msg = f"大纲防御层检测到严重错误: {str(e)}"
        print(f"[DEFENSE ERROR] {error_msg}")
        raise e


def outline_critic(state: OutlineState, config: RunnableConfig):
    """大纲审计节点"""
...
# ==========================================
def grounding_audit_node(state: OutlineState, config: RunnableConfig):
    """
...
# B 层：实体哨兵审计节点
# ==========================================
def entity_sentinel_node(state: OutlineState, config: RunnableConfig):
...
def route_after_audit(state: OutlineState):
    """
    剧本层路由决策：结合审计状态与自主化等级。
    """
    _ac = state.get("audit_count")
    ac = int(_ac) if isinstance(_ac, (int, str)) else 0
    is_approved = state.get("is_approved")
    autonomy = state.get("autonomy_level", "safe")
    
    # 逻辑审计与素材锚定有一环节失败时的重试逻辑
    if not is_approved and ac < 3:
        return "planner"
    
    # 获取审计报告中的置信评分 (假设 grounding_audit 写入了 review_log)
    # 此处逻辑同 Worldview Agent，但因为大纲更侧重创意，默认稍微严格一点
    if is_approved and autonomy in ["balanced", "aggressive"]:
        print(f"[AUTONOMY] Outline approved in {autonomy} mode. Bypassing human gate.")
        return "saver"
