...
import os
import json
from typing import List, Dict, TypedDict, Annotated, Any, Optional
...
def parse_file_node(state: ImportState, config: RunnableConfig):
    """提取文件内容"""
...
def segment_lore_node(state: ImportState, config: RunnableConfig):
    """根据策略进行切片"""
    ...
    if strategy == "regex":
        ...
        from src.common.lore_utils import parse_json_safely
...
def categorize_pga_node(state: ImportState, config: RunnableConfig):
    """分类到 PGA 0-4 架构"""
    ...
    【分类优先级指南】：
    1. 如果内容同时涉及“地点”和“运作原理/科学技术”，必须优先归类为 **Mechanisms (机制)**。
    2. 如果内容涉及“生物演化”或“社会结构”，优先归类为 **Races (种族)**。
    3. **Geographies (地理)** 仅用于纯粹的环境、气候和星体物理描述。
    """
...
def sync_library_node(state: ImportState, config: RunnableConfig):
    """同步到数据库"""
