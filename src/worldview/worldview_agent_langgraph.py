...
def autoresearch_node(state: AgentState, config: RunnableConfig):
    """
...
def generator_node(state: AgentState, config: RunnableConfig):
    ...
    if not category or category == "general":
        # 如果尚未分类，执行关键词分类
        # 1. 历史 & 战争 (History)
        if any(k in query_lower for k in ["历史", "纪元", "事件", "变迁", "战争", "编年史", "记录"]):
            category = "history"
        # 2. 势力 & 组织 (Faction)
        elif any(k in query_lower for k in ["势力", "阵营", "国家", "派系", "帝国", "联邦", "军团", "公约", "盟友", "组织", "协会", "学术", "联盟", "公司"]):
            category = "faction"
        # 3. 科技 & 机制 (Mechanism/Tech)
        elif any(k in query_lower for k in ["机制", "科技", "武器", "引擎", "原理", "技术", "装置", "协议", "热力学", "物理", "发动机", "驱动", "戴森球", "装备", "战机", "母舰", "危机", "灾难", "变故", "事故"]):
            category = "mechanism_tech"
        # 4. 种族 & 生物 (Race)
        elif any(k in query_lower for k in ["种族", "机器人", "机械族", "生命", "进化", "族群", "演化", "物种", "生物", "野兽", "怪物", "掠食者"]):
            category = "race"
        # 5. 地理 & 环境 (Geography)
        elif any(k in query_lower for k in ["地理", "地形", "环境", "星域", "坐标", "星球", "行星", "恒星"]):
            category = "geography"
        else:
            category = "general"
...
【自主化与主动性 (AI Autonomy) - 核心要求】
1. 除了满足用户的直接 Query 外，你必须**主动**在生成的 JSON 提案内部包含一个 `associated_drafts` 字段。
2. 该字段应为一个数组，包含由当前主设定衍生出的至少 1-2 个新实体（如：一个阵营的领袖，一个地点的特产物品）。
3. 这体现了你的自主推演能力，不需要用户每一次都手动下达指令。
...
注意：JSON 中必须包含 `associated_drafts` 数组，用于体现您的主动创作逻辑。
"""
...
def defense_node(state: AgentState, config: RunnableConfig):
    """
...
def route_after_review(state: AgentState):
    ...
    audit_count = int(state.get("audit_count", 0))
    confidence = float(state.get("research_confidence", 0.0))
    autonomy = state.get("autonomy_level", "safe")
    
    # 确定阈值
    threshold = {"safe": 0.99, "balanced": 0.9, "aggressive": 0.8}.get(autonomy, 0.9)
    
    if is_approved:
        # 自主化跳跃：如果置信度达到阈值，绕过人工节点直接存库
        if confidence >= threshold:
            print(f"[AUTONOMY] Confidence {confidence} >= {threshold} ({autonomy} mode). Bypassing Human Gate.")
            return "saver"
        return "human"
        
    if audit_count >= 3: 
        return "human" # 多次审计失败，强制人工干预
    return "generator"
