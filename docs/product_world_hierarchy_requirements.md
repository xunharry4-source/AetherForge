...
更新日期：2026-04-29  
适用范围：`/worlds` 世界列表、`/lore` 世界观资料库、`/outlines` 大纲章节迭代工作流、`/visualizer` 世界观图谱、后端层级 API、真实 requests 测试。
...
- **Agent工作流与循环迭代机制 (State Graph)**：
  ...
  - **创世生成节点 (Generator Node/Draft Node)**：对应五模块工作流中的 `world_agent` 草案生成步骤。
    - **LLM 扩充逻辑**：接收简略的世界名称或一句话描述，调用 LLM 扩充为包含物理法则、核心冲突、地理风貌的完整“世界摘要”。
    - **扩充规则**：遵循“摘要级”扩充（50-200字），确保生成的设定具有独特性且符合用户给出的基调。
  - **人工节点 (Approval Node/HITL)**：展示草案给用户。用户可选择：“批准 (Approve)”、“完全重写 (Full Rewrite)”、“指定局部重写 (Targeted Partial Rewrite)” 或 “指定内容重写 (Targeted Content Rewrite)”。
    - *迭代循环*：若用户选择重写或提供修改意见，则将意见写入 `feedback`，**状态流转回创世生成节点**，触发 LLM 基于反馈的二次扩充与修正。
  - **写库固化节点 (Saver/Commit Node)**：对应五模块工作流中的 `apply` 步骤，用户批准后触发，真实执行 MongoDB `worlds` 集合的写入。
- **规则说明**：
  - 世界名称必须在当前用户空间内唯一。
  - 删除世界属于高危操作，必须强制二次确认。
...
- **Agent工作流与循环迭代机制 (State Graph)**：
  ...
  - **审查节点 (Review Node)**：基于 RAG 检索校验设定是否与已有 Canon 冲突。具体审查内容包括：内容格式是否规范、是否包含逻辑漏洞、新增设定是否与该世界下已有核心设定（历史、地理、规则等）存在直接冲突。
    - *自动迭代循环*：若审查未通过，将冲突原因写入 `review_feedback`，**状态自动流转回 草案节点**，由 LLM 根据冲突意见进行“修正性扩充”，直至审查通过。
...
- **Agent工作流与循环迭代机制 (State Graph)**：
  ...
  - **草案节点 (Draft Node)**：调用 LLM 进行项目构思扩充。
    - **LLM 扩充逻辑**：基于给定的世界背景，将简短的小说标题或核心看点扩充为包含“核心主旨”、“主角轮廓”、“初期目标”的小说项目描述。
    - **扩充规则**：遵循“摘要级”扩充（100-300字），侧重于建立故事的独特性和商业吸引力。
  - **审查节点 (Review Node)**：校验小说大方向是否偏离关联的世界/世界观设定。具体审查内容包括：故事背景是否契合世界观约束、主角设定与核心主线是否符合逻辑、是否存在破坏世界基础规则的设定（反吃设定）。
    - *自动迭代循环*：如偏离设定，生成 `review_feedback` 并**状态回退至 草案节点**，要求 LLM 在特定设定约束下重新生成。
  - **人工节点 (Approval Node/HITL)**：用户确认。用户可选择：“批准 (Approve)”、“完全重写 (Full Rewrite)”、“指定局部重写 (Targeted Partial Rewrite)” 或 “指定内容重写 (Targeted Content Rewrite)”。
    - *人工迭代循环*：用户可追加修改指令 (`user_feedback`)，触发**状态回流至 草案节点**，由 LLM 完成针对性修改后重新过审。
...
- **Agent工作流与循环迭代机制 (State Graph)**：
  ...
  - **草案节点 (Draft Node)**：调用 LLM 进行结构化扩充。
  ...
  - **审查节点 (Review Node)**：审查大纲逻辑连贯性、防止剧情脱轨及设定冲突。具体审查内容包括：上下文节点剧情逻辑是否连贯、故事发展是否偏离小说主旨、核心冲突与高潮安排是否合理、是否出现与前面剧情或已有世界观设定矛盾的情节。
    - *自动迭代循环*：逻辑不通或冲突时，带有约束意见的 `review_feedback` 将**状态打回 草案节点**进行逻辑重塑。
  - **人工节点 (Approval Node/HITL)**：用户确认节点结构。用户可选择：“批准 (Approve)”、“完全重写 (Full Rewrite)”、“指定局部重写 (Targeted Partial Rewrite)” 或 “指定内容重写 (Targeted Content Rewrite)”。
    - *人工迭代循环*：长篇大纲通常需多轮微调。用户的局部修改指令会导致状态机在“特定节点重写（即特定节点的扩充）”和“审查”之间**循环迭代**，直到用户满意。
...
- **Agent工作流与循环迭代机制 (State Graph)**：
  ...
  - **审查节点 (Review Node)**：由 `review_agent` 独立执行，严查设定一致性（反吃设定）。具体审查内容包括：人物行为动机与对话是否符合已有的人设模板、场景与道具描写是否符合世界观物理法则、剧情推进与章节收尾是否严格遵循大纲约束、文字风格与视角是否存在突兀变化。
    - *自动迭代循环*：如发生吃设定，`review_agent` 给出修改建议，将流程**驳回至 草案节点**，要求 LLM 在不改变已有设定的前提下修改正文。
  - **人工节点 (Approval Node/HITL)**：提供人工介入的打磨入口。用户可选择：“批准 (Approve)”、“完全重写 (Full Rewrite)”、“指定局部重写 (Targeted Partial Rewrite)” 或 “指定内容重写 (Targeted Content Rewrite)”。
    - *人工迭代循环*：用户标记不满意段落，提交修改意见，触发**相应的重写循环**（针对该段落的二次扩充），同样需要再次经过 `review_agent` 的审查。
...
- 支持创建、编辑、删除世界，所有操作必须通过表单。
- 删除世界时必须有 `cascade` 显式确认字段；未确认且存在世界观或小说时，后端必须返回 `409`。
- 世界下属数据的管理入口由 `/lore`、`/outlines` 和 `/visualizer` 承担。

### 4.2 对话与人工介入

- 世界、世界观、小说、大纲、章节 5 个模块的新增和修改都必须进入对应的 Dify-like 工作流页面，不允许只弹普通表单直接写库。
- 创建/修改/删除均通过对话框提交给独立 Agent；其中新增和修改必须可在工作流页面中持续对话和查看节点执行状态。
- 人工介入时必须提供修改模式：
  - `full_rewrite`：完全重写。
...
### 4.4 `/outlines` 大纲章节迭代工作台

- 页面不得内置假数据，所有世界、世界观、大纲、章节数据必须来自数据库 API。
- 必须提供世界选择器，且为强制字段；默认选择第一个真实查询到的世界。
- 世界观、小说、大纲、章节默认列表显示，但除世界外不得无条件全量查询。
- 所有世界观、大纲、章节查询都必须带当前选中 `world_id`，只显示该世界内容。
- 大纲与章节的创建、修改、删除、迭代必须使用表单和 Dify-like 工作流对话框。
- 工作流节点必须可查看输入与输出，用户可在人工介入时选择 `partial_rewrite`、`full_rewrite` 或 `content_rewrite`，默认 `partial_rewrite`。
- 提供世界观导入入口：
...
- **入口要求**：
  - 5 个模块列表页中的“创建”和“修改”按钮必须进入对应模块的独立工作流页面，不允许只使用一个依赖 `type` 参数的通用页面承载全部业务。
...
- **可视化节点图**：
  - 展示工作流的完整节点链；世界必须按真实 `world_agent` 展示 `Input -> Generator/Draft -> Human -> Saver/Commit`，不得显示审查节点。
  - 世界观、小说、大纲、章节必须展示 `Input -> Draft -> Review -> Approval -> Commit`。
...
- `worldview_agent`：专用于世界观设定，包含 RAG 审查逻辑。
- `novel_agent`：专用于小说项目，包含背景契合度审查。
- `outline_agent`：专用于大纲结构，支持局部重写与剧情连贯性审查。
- `chapter_agent`：专用于章节正文，包含最严格的“反吃设定”审查。
...
- 世界：不需要审查，工作流不得显示或记录审查节点；必须记录工作流输入、真实 LLM 创世草案、人工确认和写库固化结果。
- 世界观：需要审查。
- 小说：需要审查。
- 大纲：需要审查。
- 章节：需要审查。
...
- 草案节点：展示 Agent 生成的初步方案（预览图/文本）及与原始内容的差异摘要。
- 审查节点：世界观、小说、大纲、章节必须记录审查输入、审查输出、是否通过和失败原因。
...
1. **自动迭代 (Auto-Iteration)**：
    ...
    - Agent 会自动携带审查失败原因（冲突点、逻辑漏洞）返回“草案节点”，重新调用 LLM 生成修正方案。
    - 自动迭代应有最大次数限制（默认 3 次），超过则强制转入人工介入状态。
2. **人工迭代 (Human-in-the-Loop Iteration)**：
    - 在“人工节点 (Approval Node)”中，用户通过对话框提供反馈。
    - Agent 接收反馈后，判断修改范围（`full_rewrite`, `partial_rewrite`, `content_rewrite`），并返回草案节点重新生成。
3. **固化迭代 (Commit Iteration)**：
    - 只有在人工批准后，才会通过“写库节点 (Commit Node)”完成最终固化。

#### 5.4.2 LLM 扩充规则 (Expansion Rules)

“草案节点”的核心任务是**扩充 (Expansion)**。当用户提供简略输入（如：一个标题或一句话摘要）时，LLM 必须按以下规则进行高质量扩充：
...
curl -I http://127.0.0.1:5174/outlines
...
- `tests/test_world_workflow_llm_draft_requests.py`：通过，覆盖新增世界工作流必须触发真实 `world_agent` LLM 草案节点，检查 `draft.output.llm_invoked=true`、`raw_response` 非空、世界流程不存在 `review` 节点，并在批准后查询确认真实写库。
...
- 仍需补充 E2E 级点击测试，逐项模拟页面按钮点击、启动工作流、人工反馈、批准和业务列表回查；当前已由真实 requests 测试覆盖 API 写库链路，并由浏览器检查覆盖 5 个独立页面可访问与核心控件可见。
...
- 工作流完整性：世界、世界观、小说、大纲、章节 5 个模块的新增和修改共 10 条链路，都必须有可访问的 Dify-like 独立工作流页面。
...
- 页面安全性：`/worlds` 只展示世界列表；`/lore`、`/outlines`、`/visualizer` 必须强制选择世界并只显示该世界数据。
- 导入正确性：世界观 JSON、Markdown、OPML 导入必须保留上下级关系，并由真实查询确认。
- 测试真实性：真实 requests API 测试通过。
- 前端可用性：`/worlds`、`/workflow/world`、`/workflow/worldview`、`/workflow/novel`、`/workflow/outline`、`/workflow/chapter`、`/lore`、`/outlines`、`/visualizer` 可访问，前端构建通过。
