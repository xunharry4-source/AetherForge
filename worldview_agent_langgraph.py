...
CATEGORY_LOGIC_TEMPLATES = {
    ...
    "history": {
        "title": "历史逻辑 (History)",
        "logic": """
        1. 线性一致性：按时间轴顺序确定的因果链条，严禁出现时间旅行或溯因性修改。
...
def generator_node(state: AgentState):
    ...
    if not query:
        print("[WARNING] 'query' is missing in state at generator_node!")
        
    query_lower = query.lower()
    category = "general"
    category_info = {"title": "一般世界观", "logic": "遵循PGA底层物理与逻辑。"}
    ...
    category_info = CATEGORY_LOGIC_TEMPLATES.get(category, category_info)
    
    # 2. 获取分类模板 (MongoDB/Local Fallback)
    template_data = get_category_template(category)
    if not template_data and category != "general":
        # 如果模板不存在，先生成一个
        meta_prompt = f"""你是一个世界观架构师。请为【{category}】这个分类创建一个标准的 JSON 模板和参考例子。
要求：
1. 包含核心属性（如名称、描述、物理参数、PGA协议合规性等）。
2. 提供一个符合硬科幻质感的参考例子。
3. 必须输出为合法有效的 JSON 格式。
"""
...
【思维链要求 (THINKING)】
- 确认反馈：我是否已经根据“当前核心修改需求”进行了调整？
- 确认分类：当前任务是“{category_info['title']}”。
- 格式契合：我是否完整填充了模板要求的 JSON 字段？
- 架构一致性：新内容的加入是否符合 0-4 逻辑链条？
...
TASK: 请根据以上要求（特别是【当前核心修改需求】）产出一个“设定提案”。必须输出为 JSON 格式。
"""
    ...
    return {
        ...
        "status_message": f"[{category_info['title']}] 提议已基于模板生成，正在提交逻辑审查..."
    }


def reviewer_node(state: AgentState):
    print(f"[DEBUG] reviewer_node entry. State keys: {list(state.keys())}")
    print(f"\n[DEBUG] Entering reviewer_node (proposal: {state.get('proposal', '')[:50]}...)")
    category = state.get('category', 'general')
    category_info = CATEGORY_LOGIC_TEMPLATES.get(category, {"title": "一般世界观", "logic": "遵循PGA底层物理与逻辑。"})
    
    prohibited_items = get_prohibited_rules()
    worldview_rules = get_worldview_context_by_category(state.get('query', ''))
    
    full_prompt = f"""你是一个负责维护 PGA 世界观严谨性的“逻辑审核官”。
你必须将审核结果输出为 **JSON 格式**。

【审核标准：{category_info['title']}】
该设定必须符合以下分类逻辑，禁止跨类别混淆：
{category_info['logic']}
...
【硬性检查点】
1. 逻辑隔离：(重点检查) 是否出现了“挂羊头卖狗肉”的情况？例如“创建势力”变成了“描写某种族生理”。
2. 物理合规：是否违反了热力学基本定律？
3. 架构锚点：是否正确放置在 PGA 的 0-4 系统中？

【输出格式】
{{
  "status": "合理" 或 "不合理",
  "audit_log": "详细的记录。如果发现分类错位（如势力写成种族），必须明确指出冲突。",
  "category_purity": "纯粹" 或 "混淆"
}}
...
"""
    ...
    try:
        ...
        msg = f"完成第 {count + 1} 次逻辑审计：{'通过' if is_ok else '检测到逻辑混淆，正在重试修正'}"
        ...
        print(f"[DEBUG] Reviewer parsing error: {e}, raw content: {res.content}")
        return {"review_log": res.content, "is_approved": False, "audit_count": count + 1, "status_message": "审核解析异常"}

def human_node(state: AgentState):
    print(f"[DEBUG] human_node entry. State keys: {list(state.keys())}")
    print(f"\n[DEBUG] Entering human_node (current_status: {state.get('status_message')})")
    if os.getenv("AGENT_MODE") == "CLI":
        print(f"\n--- [Agent {state.get('category','').upper()} 提议] ---\n{state['proposal']}")
    ...
    user_input = interrupt({"status_message": f"{state.get('category','').upper()} 设定已就绪，正在等待核准...", "proposal": state['proposal']})
    print(f"[DEBUG] human_node: Resumed. Received feedback: '{user_input}'")
    # user_input 是通过 Command(resume=...) 传入的字符串
    return {"user_feedback": user_input, "is_approved": user_input == "批准", "status_message": "已收到反馈，正在处理..."}
